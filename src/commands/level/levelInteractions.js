...
async function handlePage(interaction) {
  ...
  // 修正: offset を渡して正しいユーザーを取得
  const topUsers = await getTopUsers(interaction.guild.id, 30, offset); 
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`level_page_${page + 1}`).setLabel('›').setStyle(ButtonStyle.Secondary).setDisabled(topUsers.length < 30)
  );
